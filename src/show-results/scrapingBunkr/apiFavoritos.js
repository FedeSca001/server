...
const actualizarContador = (elemento, cantidad, texto) => {
    if (elemento) elemento.textContent = `${texto}: ${cantidad}`;
};
...
const botonFavorito = (tipo, id) => {
    return esFavorito(tipo, id)
        ? `<button type="button" class="btn-fav" data-tipo="${tipo}" data-id="${id}" data-action="delete">⭐</button>`
        : `<button type="button" class="btn-fav" data-tipo="${tipo}" data-id="${id}" data-action="add">☆</button>`;
};
...
const deleteFavorito = async id => {
    try {
        ...
        const response = await fetch(`${API_FAVORITOS}/${encodeURIComponent(id)}`, {
            method: "DELETE"
        });
        if (!response.ok) {
